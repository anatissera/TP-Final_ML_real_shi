...
def beta_triang(epoch, cycle=10, beta_max=4.0):
    phase = epoch % cycle
    return beta_max * (1 - abs((phase/(cycle/2)) - 1))

def beta_cosine(epoch, epochs, beta_max=4.0):
    return beta_max * 0.5 * (1 + math.cos(math.pi * epoch / epochs))

def beta_inv(epoch, epochs, beta_max=4.0):
    return beta_max * (1 - (epoch/epochs))
...
class VAE1D(nn.Module):
    def __init__(self, input_ch=1, coeff_ch=55, latent_dim=16, seq_len=2048, n_blocks=3):
        ...
        # Encoder for the time-series ECG signal
        ...
        # Dense layers that will process the COMBINED features
        ...
        # Decoder part (reconstructs only the ECG signal)
    ...
    def encode(self, x_signal, x_coeffs):
        # 1. Process the time-series signal through the CNN encoder
        h_signal = self.signal_encoder(x_signal)
        h_signal_flat = self.flatten(h_signal)
        
        # 2. Concatenate the flattened signal features with the static FMM coefficients
        h_combined = torch.cat([h_signal_flat, x_coeffs], dim=1)
        
        # 3. Project the combined features into the latent space
    ...
    def decode(self, z):
        # The decoder only needs to reconstruct the ECG signal
        h_decoded = self.fc_dec(z)
        return self.decoder(h_decoded)

    def forward(self, x_signal, x_coeffs):
        # This is the full forward pass for training
...
def compute_scores(model, loader, device, beta):
    ...
    errs, zs = [], []
    with torch.no_grad():
        # The loader now yields signal, coeffs, and a label (which we ignore here, '_')
        for x_signal, x_coeffs, _ in loader:
            x_signal, x_coeffs = x_signal.to(device), x_coeffs.to(device)
            
            # The model's encode function now takes two arguments
            mu, logv = model.encode(x_signal, x_coeffs)
            z = model.reparameterize(mu, logv)
            rec = model.decode(z)
            
            # The reconstruction error is still calculated against the original signal
            mse = ((rec - x_signal)**2).mean(dim=[1, 2]).cpu().numpy()
            kl = (-0.5 * (1 + logv - mu.pow(2) - logv.exp()).sum(dim=1)).cpu().numpy()
            
            # The ELBO score is a combination of reconstruction and KL divergence
            elbo = -mse - beta * kl
